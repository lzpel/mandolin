...
impl Mandolin {
    ...
    fn p<'a>(api: &'a Value, path: &str, no_err: bool) -> Result<&'a Value, Error> {
        let default = if no_err {
            ...
            Err(tera::Error::from(format!("p: {path} not found")))
        };
        let mut parent = api;
        for p in path.split("/").skip(1) {
            ...
            parent = if let serde_json::Value::Object(map) = parent {
                ...
                match p.parse::<usize>() {
                    Ok(v) => match v < array.len() {
                        //removeでは他の要素の前詰めで遅いのでswap_removeで高速化
    ...
    fn ls<'a>(
        ...
        path: &str,
        no_err: bool,
    ) -> Result<BTreeMap<String, &'a Value>, Error> {
        let v = match Self::pr(api, path, no_err)? {
            ...
            _ => {
                return if no_err {
                    ...
                    Err(tera::Error::from(format!("ls {}", path)))
                }
            }
        };
        Ok(v)
    }
    fn lsop<'a>(api: &'a Value, path: &str, no_err: bool) -> Result<BTreeMap<String, &'a Value>, Error> {
        let v = Self::ls(api, path, no_err)?;
		let methods=["get", "put", "post", "delete", "options", "head", "patch", "trace"];
        let w = v
            .into_iter()
            .map(|(k, v_path)| {
                Self::ls(api, k.as_str(), no_err).unwrap_or_default()
                    .into_iter()
                    .map(move |(k, v)| (k, v_path, v))
            })
            .flatten()
            .filter(|(k, _, _)| methods.iter().any(|v| k.ends_with(v)))
            .map(|(k, v, w)| (k, w))// 本来はここで親元のparametersを合成することができるはず
            .collect();
        Ok(w)
    }
    fn value_cloned<'a, M: IntoIterator<Item = (String, &'a Value)>>(m: M) -> Value {
        Value::Object(m.into_iter().map(|(k, v)| (k, v.clone())).collect())
    }
    pub fn render(&self) -> Result<String, tera::Error> {
        ...
        });
        {
            ...
            tera.register_filter(
                ...
                move |value: &tera::Value, _: &HashMap<String, tera::Value>| {
                    Self::p(&api, value.as_str().unwrap_or_default(), true).cloned()
                },
            );
        }
        {
        ...
        }
        {
            ...
            tera.register_filter(
                "pr",
                move |value: &tera::Value, _: &HashMap<String, tera::Value>| {
                    Self::pr(&api, value.as_str().unwrap_or_default(), true).cloned()
                },
            );
        }
        {
            ...
            tera.register_filter(
                "ls",
                move |value: &tera::Value, _: &HashMap<String, tera::Value>| {
                    Self::ls(&api, value.as_str().unwrap_or_default(), true).map(|v| Self::value_cloned(v))
                },
            );
        }
        {
            ...
            tera.register_filter(
                "lsop",
                move |value: &tera::Value, _: &HashMap<String, tera::Value>| {
                    Self::lsop(&api, value.as_str().unwrap_or_default(), true).map(|v| Self::value_cloned(v))
                },
            );
        }
        tera.register_filter(
            ...
            |value: &tera::Value, _: &HashMap<String, tera::Value>| {
                ...
                let v = match i {
                    ReferenceOr::Reference { reference } => {
                        reference.replace("#/components/schemas/", "").to_string()
                    }
                ...
                let operations: Vec<LappedOperation> = paths
                    ...
                    .map(|(path_key, path)| {
                        ...
                        .map(|(method, operation)| {
                            LappedOperation::new(path_key, method, &operation.unwrap())
                        })
                    })
                    .flatten()
                    .collect();
